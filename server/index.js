...
const upload = multer({ 
  ...
  // Увеличиваем лимит для поддержки ZIP-архивов (налоговая/финансовая отчетность)
...
const categorizeUploadedFile = (originalName, mimeType) => {
  ...
  if (isZip) {
    if (name.includes('nalog') || name.includes('налог')) return 'taxes'
    if (name.includes('fin') || name.includes('finance') || name.includes('фин')) return 'financial'
    // Если zip, но не удалось определить — оставим null, уточним в диалоге
    return null
  }
  // По умолчанию считаем PDF/табличные как банковские выписки
  return 'statements'
}
...
2. ПРОВЕРКА ПЕРИОДА:
   ...
   ПОСЛЕ получения второй выписки другого банка за тот же период, что первая выписка:
      ...
      - Если ДА: "Выписка принята. Есть ли у вас счета в этом или других банках? Если да, прикрепите выписки за 12 месяцев. Если нет, напишите 'нет'."
  ...
  try {
    ...
    if (file) {
      ...
      try {
        ...
        // Категоризируем и сохраняем метаданные файла в БД
        const category = categorizeUploadedFile(file.originalname, file.mimetype)
        await saveFileToDB(session, uploadedFileId, file.originalname, file.size, file.mimetype, category)
    ...
    console.log(`📚 История для агента: ${history.length} сообщений`)
      ...
      const timeoutPromise = new Promise((_, reject) => 
        setTimeout(() => reject(new Error('Agent timeout (300s)')), 300000)
      )
