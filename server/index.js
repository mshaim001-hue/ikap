...
const categorizeUploadedFile = (originalName, mimeType) => {
  ...
  if (isPdf) {
    ...
    if (name.includes('fin') || name.includes('finance') || name.includes('фин') || name.includes('balance') || name.includes('баланс')) {
      return 'financial'
    }
  ...
  // Если не PDF - вернем null (будет обработано агентом)
  return null
}
...
- После получения обеих PDF-файлов — попроси контактные данные: имя, фамилия, email, телефон.
      
ВАЖНО: НЕ ПЕРЕХОДИ к контактам БЕЗ явного "нет"!
И НЕ ПЕРЕХОДИ к контактам БЕЗ получения обеих PDF-отчетностей: налоговой и финансовой.

РАБОТА С PDF-ФАЙЛАМИ:
- Принимай ТОЛЬКО PDF файлы (mimetype application/pdf). Если пользователь прикрепил PDF, подтверди прием: "PDF принят" или "Отчетность принята".
- Если прикреплен не PDF — вежливо попроси прислать именно PDF файл.
- Все документы должны быть в формате PDF: выписки, налоговая отчетность, финансовая отчетность.
