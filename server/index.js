...
const buildPool = () => {
  // Используем Supabase Connection Pooling (PgBouncer) для обхода IPv6 проблем
  ...
  // Если это обычный URL, конвертируем в pooler
  const poolerHost = url.hostname.replace('db.', 'aws-0-').replace('.supabase.co', '.pooler.supabase.com')
